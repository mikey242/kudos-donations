...
class MailerService extends AbstractService {
	...
	public function __construct() {
		...
		$this->from = "From: Kudos " . __('Donations', 'kudos-donations') . ' <' . (Settings::get_setting('smtp_from') ?: Settings::get_setting('smtp_username')) . '>';
		parent::__construct();
	...
	public function init($phpmailer) {
		...
		$phpmailer->addEmbeddedImage(Utils::get_asset_url('img/logo-colour-40.png', true), 'kudos-logo', 'kudos-logo.png');
	...
	public function send_receipt($transaction) {
		...
		$this->send($transaction->get_donor()->email, __('Kudos Donation Receipt', 'kudos-donations'), $body, $headers, $attachments);
	}
