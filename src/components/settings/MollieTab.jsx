...
const MollieTab = ({ checkApiKeys }) => {
	...
	const checkKey = (name, key) => {
		if(key) {
			...
			checkApiKeys({[name]: key}).then(() => setCheckingKey({[name]: false}))
		}
	}
	...
	return (
		<Fragment>
			<RadioGroupControl
				...
				options={[
					{ label: __('Test', 'kudos-donations'), value: 'test', disabled: !test?.verified },
					{ label: __('Live', 'kudos-donations'), value: 'live', disabled: !live?.verified },
				]}
				help={__(
			...
			<Divider />
			<>
				{[{...watchLive, 'name': 'live'}, {...watchTest, 'name': 'test'}].map(( mode, i ) => {
					return (
						<TextControl
							key={i}
							...
							label={<span className='capitalize'> {mode?.name} key</span>}
							inlineButton={mode?.verified ? (<CheckCircleIcon className={'text-green-600 w-6 h-6'} />) : (<Button isOutline onClick={() => checkKey(mode.name, mode?.key)}>{__('Apply', 'kudos-donations')}</Button>)}
						/>
					)
				})}
				<p role={'button'} className='ml-auto text-red-600 underline text-right cursor-pointer block' onClick={() => updateSetting('_kudos_vendor_mollie', {'mode': 'test'})}>{__('Reset Mollie', 'kudos-donations')}</p>
			</>
		</Fragment>
	);
};
