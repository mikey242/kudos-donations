...
export function createSummary(formId) {
	...
	if (frequency === 'recurring') {
		...
		const length = form.querySelector(
			"[name='recurring_length'] option[value='" + recurring_length + "']"
		).innerHTML;
		const frequency = form.querySelector(
			"[name='recurring_frequency'] option[value='" +
				recurring_frequency +
				"']"
		).innerHTML;
		type =
...
export function valueChange(form) {
	form.addEventListener('change', (e) => {
		...
		if (progressBar) {
			...
			const extra = form
				.querySelector('.kudos-campaign-progress')
				.querySelector('.kudos-progress-extra');
			if (e.target.name === 'value') {
				const value = Number.isInteger(parseFloat(form.value.value))
					? parseFloat(form.value.value)
					: 0;
