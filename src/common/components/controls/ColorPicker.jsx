        ...
        <Controller
            ...
            render={({ field: { onChange, value } }) => (
                <RadioGroup value={value} onChange={onChange} className="mt-3">
                    <RadioGroup.Label
                        className="block text-sm font-medium font-bold text-gray-700">{label}</RadioGroup.Label>
                    ...
                    <div className="mt-4 inline-flex items-center space-x-3">
                        ...
                        <>
                            ...
                            <RadioGroup.Option
                                ...
                                value={value}
                                className={({ active }) => {
                                  const checked = !colors.filter(color => color.value === value).length > 0
                                  return (classNames(
                                    active && checked ? 'ring ring-offset-1' : '',
                                    !active && checked ? 'ring-2' : '',
                            ...
                            >
                                ...
                                {showPicker &&
                                    <div
                                        className="absolute left-full bottom-full z-1050">
                                        <div
                                            className="bg-white mt-2 p-5 relative rounded-lg drop-shadow-md z-[2]">
                                            <HexColorInput
                                                className={'w-20 border-gray-300 mt-2 placeholder-gray-500 border border-solid transition ease-in-out duration-75 leading-6 text-gray-700 bg-white focus:border-primary focus:outline-none focus:ring-0 py-2 px-3 rounded w-full'}
                                                color={value} onChange={onChange} prefixed/>
                                        </div>
